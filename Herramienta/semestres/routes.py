...
from Herramienta.semestres.forms import CrearSemestreForm, EditarNombreSemestreForm, AgregarCursoASemestreForm
...
def agregarCurso_semestre(semestre_id):
    ...
    cursos = [(c.id, c.nombre) for c in Curso.query.all() if c not in cursosActuales]
    ...
    return render_template("semestres/anadirCurso_semestre.html", title="Anadir curso a semestre", form=form, semestre=semestre)